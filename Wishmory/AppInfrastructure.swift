...
private struct AwwRefreshControlBranding: UIViewRepresentable {
    ...
    final class Coordinator: NSObject {
        ...
        func attach(to view: UIView) {
            DispatchQueue.main.async { [weak self] in
                ...
                self.refreshStateObservation = refreshControl.observe(
                    ...
                    if isRefreshing {
                        self?.prepareNextMessage()
                        AwwHaptics.refreshStarted()
                    } else {
                        self?.hideMessage()
                        AwwHaptics.refreshCompleted()
                    }
                }
            }
        }
        ...
        private func prepareNextMessage() {
            ...
            // Keep the message in the refresh control's header only while it is
            // active. Clearing it after refresh lets UIKit collapse that header.
...
private enum AppIconQuickAction {
    static let feedbackURL = URL(string: "https://forms.gle/2QEjaYCzsRoQsJtEA")

    nonisolated static func isFeedbackAction(_ shortcutItem: UIApplicationShortcutItem) -> Bool {
        shortcutItem.type == "com.awwlist.feedback-before-leaving"
    }
}

private final class AwwSceneDelegate: NSObject, UIWindowSceneDelegate {
    private var shouldOpenFeedbackForm = false
    ...
    func scene(
        ...
        shouldOpenFeedbackForm = connectionOptions.shortcutItem
            .map(AppIconQuickAction.isFeedbackAction) ?? false
    }

    func sceneDidBecomeActive(_ scene: UIScene) {
        openFeedbackFormIfNeeded()
    }
    ...
    func windowScene(
        ...
        guard AppIconQuickAction.isFeedbackAction(shortcutItem) else {
            completionHandler(false)
            return
        }

        shouldOpenFeedbackForm = true
        openFeedbackFormIfNeeded()
        completionHandler(true)
    }

    private func openFeedbackFormIfNeeded() {
        guard shouldOpenFeedbackForm,
              let feedbackURL = AppIconQuickAction.feedbackURL
        else {
            return
        }

        shouldOpenFeedbackForm = false
        UIApplication.shared.open(feedbackURL)
    }
}

private final class AwwAppDelegate: NSObject, UIApplicationDelegate {
    func application(
        ...
        if connectingSceneSession.role == .windowApplication {
            configuration.delegateClass = AwwSceneDelegate.self
        }
...
struct AwwListApp: App {
    @UIApplicationDelegateAdaptor(AwwAppDelegate.self) private var appDelegate
